...
clc
clear all
Initialprodrate= 613; %total prod
Initialwatercut= 0.56;
InitialGOR=446;
Oilproddeclinerate=12;
Totalabandonmentrate=800;
Abandonementwatercut=0.65;
...
R = log(1-Rdecl);
%Starting loop to calculate costs and net present value up to the abandonement year
noofdaysperworkover=5;
noofworkoversperyear=2;
Abandonement_GOR=500;
cummulative_NPV=0;
...
Water_cut_I= Watercut_yearzero + I * (Abandonementwatercut - Initialwatercut)/Yearstoabandonement
GOR_I= GOR_yearzero +I *(Abandonement_GOR - InitialGOR)/Yearstoabandonement;
...
ElectcostperKwhr=0.025;
ElectcostinkWperliquidprod=20;
...
averagecostofcomponentreplacement=2000;
...
xlswrite('NFLiftNPVAnalysis.xlsx',Parameters,'MatlabgenNFvalues','A2')
end